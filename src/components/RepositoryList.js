...
const RepositoryHeader =
  ...
  style = {
    {
      display: `flex`, justifyContent: `space-between`, margin: 0, fontSize: 20,
    }
  } > <ExternalLink url = {`https://github.com${repo.resourcePath}`}>{
          repo.name}</ExternalLink>
      </h4>< GitHubButton
  href = {`https://github.com${repo.resourcePath}`} dataIcon = "octicon-star"
  dataSize = "large"
  ...
  return (
    <div style={{ color: `#586069`, fontSize: 12 }}>
      <div style={{ margin: "1em 0" }}>
        ...
        (<FooterItem>
         <Octicon icon =
          { Law } /> {repo.licenseInfo.name}
        </FooterItem>)}<FooterItem>
         Created: {createdAt}</FooterItem>
    </div>)
}
...
const RepositoryDescription = ({repo}) =>
    (<div style = {
      ...
      { color: `#586069`, marginBottom: 0 }
    }>{repo.description && repo.description}

     {repo.homepageUrl &&
      (<>{" -"} {" "}<ExternalLink url = {repo.homepageUrl}>{
          repo.homepageUrl}</ExternalLink>
        </>)}</p>
  </div>)
...
const Repository =
  ...
  return (
    <div
      style={{
    borderBottom: `1px solid #e1e4e8`, padding: `0 1rem 1rem 1rem`,
        fontSize: 16,
      }}
    >
...
const RepositorySectionMaker = ({data}) =>
    (<div className = "grids smd-2 col-1">{data}<
        /div>
)
        ...
        const repos = data.githubData.data.viewer.repositories.nodes.map(
          repo => {
            return repo.name !== "msichterman" ? (
              <Repository key={repo.id} repo={repo} />): null
}
        )
